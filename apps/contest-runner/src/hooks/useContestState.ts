...
import type { ContestState, ContestAction, Caller, QsoState } from '../types';
...
function contestReducer(state: ContestState, action: ContestAction): ContestState {
  switch (action.type) {
    ...
    case 'LOG_QSO': {
      // Need a caller selected and a serial number entered
      if (!state.selectedCaller || !state.enteredNr) {
        return state;
      }
