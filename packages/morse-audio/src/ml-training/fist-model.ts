 ...
 */
export const FIST_DISTRIBUTION: Array<{ profile: FistProfile; probability: number }> = [
  { profile: 'machine', probability: 0.10 },
  { profile: 'good', probability: 0.25 },
  { profile: 'average', probability: 0.35 },
  { profile: 'poor', probability: 0.20 },
  { profile: 'very_poor', probability: 0.10 },
];
 ...
 */
export function randomFistProfile(prng: () => number = Math.random): FistProfile {
  const r = prng();
  let cumulative = 0;
...
export function applyFistModel(
  ...
  const opts: Required<FistOptions> = {
    jitter: options.jitter ?? 0,
    dahBias: options.dahBias ?? 0,
    speedDriftWpmPerSec: options.speedDriftWpmPerSec ?? 0,
    charGapStretchFraction: options.charGapStretchFraction ?? 0,
    charGapStretchRange: options.charGapStretchRange ?? [1, 1],
  };
  ...
  for (let i = 0; i < originalTimings.length; i++) {
    ...
    currentWpmDrift += driftDirection * opts.speedDriftWpmPerSec * (originalDuration / 1000);
    ...
    if (element.elementType === 'dah') {
      // Apply dah bias
      newDuration *= (1 + dahBias + opts.dahBias);
    }

    // Apply Gaussian jitter
    const jitterStdDev = newDuration * opts.jitter;
    ...
    if (element.elementType === 'char_gap' && charGapIndices.includes(i)) {
      const [minStretch, maxStretch] = opts.charGapStretchRange;
  ...
  for (const elem of newElements) {
    if (elem.elementType === 'word_gap') {
      ...
      if (currentChar) {
        newCharacters.push({ char: currentChar, startMs: charStartMs, endMs: charEndMs });
        currentChar = '';
      }
      continue;
    }
  ...
  const originalTotalMs = originalTimings.reduce((sum, t) => sum + Math.abs(t), 0);
